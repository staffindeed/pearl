...
func generateRPCKeyPair(writeKey bool) (tls.Certificate, error) {
	...
	keyPair, err := btcutil.WriteTLSCertPair(cfg.RPCCert.Value,
		cfg.RPCKey.Value, org, validUntil, nil, writeKey)
	if err != nil {
		return tls.Certificate{}, err
	}

	if writeKey {
		log.Infof("Done generating TLS certificate %q and key %q",
			cfg.RPCCert.Value, cfg.RPCKey.Value)
	} else {
		log.Infof("Done generating TLS certificate %q (one-time key "+
			"not stored)", cfg.RPCCert.Value)
	}
	return keyPair, nil
}
...
func startRPCServers(walletLoader *wallet.Loader) (*grpc.Server, *legacyrpc.Server, error) {
	...
	if cfg.DisableServerTLS {
		...
		tlsConfig := &tls.Config{
			...
			// Pin HTTP/1.1 via ALPN.  The legacy RPC server serves
			// websockets, which require the HTTP/1.1 Upgrade handshake
			// (HTTP/2 has no equivalent here), and its POST handler
			// relies on HTTP/1.1 connection-close semantics.
